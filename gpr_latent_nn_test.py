...
def interp_nn(vae, interp_net, input1, input2, label, args):
    ...
    with torch.no_grad():
        ...
        for i in range(len(all_eps1)):
            ...
            z_out = np.empty(z1.shape)
            for j in range(dims[1]):
                for k in range(dims[2]):
                    x = torch.tensor(list(z1[:, j, k]) + list(z2[:, j, k]) + [label], dtype=torch.float)
                    z_out[:, j, k] = interp_net(x)  # Interpolate

            z_out = torch.tensor(z_out, dtype=torch.float).unsqueeze(0).cuda()
            all_z.append(z_out)
...
def main(args):
    ...
    with torch.no_grad():
        ...
        all_eps1, all_z1 = vae.calculate_eps_and_z(im1)
        n_channels = len(all_z1[0].cpu().numpy()[0, :, 0, 0])

    interp_net = InterpNet(n_channels)
