...
def get_loaders_eval(dataset, root, distributed, batch_size, augment=True, drop_last_train=True, shuffle_train=True,
    ...
    if dataset == 'cifar10':
        ...
        if dataset == 'celeba_64':
            ...
            valid_data = LMDBDataset(root=root, name='celeba64', train=False, transform=valid_transform,
                                     is_encoded=True)
        ...
        num_classes = 0
        train_transform, valid_transform = _data_transforms_gpr()
        train_data = LMDBDataset(root=root, name='gpr', train=True, transform=train_transform)
        valid_data = LMDBDataset(root=root, name='gpr', train=False, transform=valid_transform)
    else:
        raise NotImplementedError
