...
from gpr_lsgm_1to1_test import gpr_lsgm_1to1_test
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        if epoch + 1 in milestones_epochs or epoch == (args.epochs - 1):
            ...
            neg_log_p_ode, _, nfe_nll_ode, _, _ = elbo_evaluation(valid_queue, diffusion_cont, dae, args, vae,
                                                                  max_step=10, ode_eval=True,
                                                                  ode_param_dict=fast_ode_param)
            logging.info('valid NLL (ODE-based) {} bpd (with {} func. evaluations)'.format(neg_log_p_ode * bpd_coeff,
                                                                                           nfe_nll_ode))
            ...
            json_common.append({
                ...
                "ode_nll_bpd": float(neg_log_p_ode * bpd_coeff),
                "ode_nll_nat": float(neg_log_p_ode),
                "ode_nll_nfe": float(nfe_nll_ode)
            })

            mae_normal, ssim_normal, psnr_normal = gpr_lsgm_1to1_test(ds.normal_interp_part, vae, dae, diffusion_cont,
                                                                      args.train_ode_eps, args.train_ode_solver_tol,
                                                                      "linear", shape, args, verbose=False)
            mae_extr, ssim_extr, psnr_extr = gpr_lsgm_1to1_test(ds.extrapolation_part, vae, dae, diffusion_cont,
                                                                args.train_ode_eps, args.train_ode_solver_tol,
                                                                "linear", shape, args, verbose=False)
            ...
            """
            current_ssim = np.mean(ssim_normal) + np.mean(ssim_extr)
            if current_ssim > best_ssim:
                best_ssim = current_ssim
                print("SSIM: {}".format(current_ssim))
                if args.global_rank == 0:
                    # Because we already swapped EMA parameters, we only save the VAE/DAE models here.
                    logging.info('saving the model for SSIM.')
                    checkpoint_file_ssim = os.path.join(args.save, 'checkpoint_ssim.pt')
                    content = {'epoch': epoch + 1, 'global_step': global_step, 'args': args,
                               'best_score_fid': best_score_fid, 'best_score_nll': best_score_nll,
                               'dae_state_dict': dae.state_dict(), 'vae_state_dict': vae.state_dict()}
                    torch.save(content, checkpoint_file_ssim)
