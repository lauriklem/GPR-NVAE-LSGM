...
from gpr_vae_3to1_test import gpr_vae_3to1_test
...
def main(args):
    ...
    ds = GPRDataset(stdev=0, datadir="./datasets/gpr_pics", images_between=[4],
                    verbose=True, onlysim=True, simulated_folders_train=sim_train, simulated_folders_test=sim_test)
    train_ind = ds.training_sim
    valid_ind = ds.testing_sim

    # Form training and testing datasets
    train_queue = []
    for row in train_ind:
        ...
        gt = Image.open(gt)
        gt = np.array(gt.getdata(), dtype="float32").reshape((gt.size[1], gt.size[0], 3)) / 128.0 - 1
        train_queue.append([x, gt])

    valid_queue = []
    for row in valid_ind:
        input1, input2, gt, label = row
        im1 = Image.open(input1)
        lblimg = np.full(fill_value=label * 2 - 1, shape=(im1.size[1], im1.size[0], 1), dtype="float32")
        im1 = np.array(im1.getdata(), dtype="float32").reshape((im1.size[1], im1.size[0], 3)) / 128.0 - 1
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        # Evaluate at milestone epochs
        if epoch + 1 in milestones_epochs or epoch == (args.epochs - 1):
            vae.eval()
            ...
            json_common.append({
                "epoch": epoch + 1,
                "train_loss": float(train_obj.item()),
                "bpd_log_p": float(valid_neg_log_p * bpd_coeff),
                "nat_log_p": float(valid_neg_log_p),
                "bpd_elbo": float(valid_nelbo * bpd_coeff),
                "nat_elbo": float(valid_nelbo)
            })

            # test model for interpolation
            ...
            json_metrics.append({
                "epoch": epoch + 1,
                "mae_normal": float(np.mean(mae_normal)),
                "mae_extr": float(np.mean(mae_extr)),
                "ssim_normal": float(np.mean(ssim_normal)),
                "ssim_extr": float(np.mean(ssim_extr)),
                "psnr_normal": float(np.mean(psnr_normal)),
                "psnr_extr": float(np.mean(psnr_extr))
            })
        ...
        if (args.global_rank == 0 and epoch in milestones_epochs and epoch > 0) or epoch == (args.epochs - 1):
    ...
    f = open(args.save + "/acc_common", "w")
    json.dump(json_common, f, indent=4)
    f.close()

    f = open(args.save + "/acc_metrics", "w")
...
def train_vae(args, train_queue, model, optimizer, grad_scalar, global_step, warmup_iters, writer, logging, sn_calculator):
    ...
    model.train()
    for i in range(0, len(train_queue), args.batch_size):
        x_batch = None
        gt_batch = None
        for j in range(args.batch_size):
            ...
            gt = trans(gt).unsqueeze(0)

            if x_batch is None:
                x_batch = x
            else:
                x_batch = torch.vstack((x_batch, x))
            if gt_batch is None:
                gt_batch = gt
            else:
                gt_batch = torch.vstack((gt_batch, gt))

        x = utils.common_x_operations_cond(x_batch)
        gt = utils.common_x_operations_cond(gt_batch)
        ...
        if (global_step + 1) % 100 == 0:
            ...
            writer.add_scalar('train/recon_iter', torch.mean(
                utils.reconstruction_loss(output, gt, crop=model.crop_output)), global_step)
...
def test_vae(valid_queue, model, num_samples, args, logging):
    ...
    trans = transforms.Compose([transforms.ToTensor()])
    model.eval()
    for step in range(0, len(valid_queue), args.batch_size):
        x_batch = None
        gt_batch = None
        for j in range(args.batch_size):
            row = valid_queue[step + j]
            x, gt = row
            x = trans(x).unsqueeze(0)
            gt = trans(gt).unsqueeze(0)
        ...
        with torch.no_grad():
            ...
            for k in range(num_samples):
                ...
                log_iw.append(utils.log_iw(output, gt, log_q, log_p, crop=model.crop_output))
