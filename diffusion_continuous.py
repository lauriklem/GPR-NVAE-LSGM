...
class DiffusionBase(ABC):
    ...
    def reverse_generative_ode(self, dae, eps, ode_eps, ode_solver_tol, enable_autocast, no_autograd):
        """
        Solve forward ODE. This has been modified from the method sample_model_ode.
        """
        # ODE solver starts consuming the CPU memory without this on large models
        # https://github.com/scipy/scipy/issues/10070
        gc.collect()
        ...
        def ode_func(t, state):
            ...
            with torch.set_grad_enabled(True):
                with autocast(enabled=enable_autocast):
                    variance = self.var(t=t)
                    mixing_component = self.mixing_component(x_noisy=x, var_t=variance, t=t,
