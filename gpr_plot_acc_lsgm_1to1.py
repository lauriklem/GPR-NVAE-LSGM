...
def get_accs_extr(path):
    ...
    for acc_type in ["common", "metrics"]:
        ...
        if acc_type == "common":
            ode_nll_bpd, ode_nll_nat, ode_nll_nfe, train_loss = [], [], [], []
            for elem in list1:
                ode_nll_bpd.append(elem["ode_nll_bpd"])
                ode_nll_nat.append(elem["ode_nll_nat"])
                ode_nll_nfe.append(elem["ode_nll_nfe"])
                train_loss.append(elem["train_loss"])
            common = [ode_nll_bpd, ode_nll_nat, ode_nll_nfe, train_loss]
...
if __name__ == "__main__":
    ...
    path = "./checkpoints/{}/lsgm".format(model)
    common, metrics, epochs = get_accs_extr(path)
    ode_nll_bpd, ode_nll_nat, ode_nll_nfe, train_loss = common
