...
from gpr_vae_1to1_test import gpr_vae_1to1_test, vae_recon_test
...
def main(args):
    ...
    json_linear, json_spherical, json_common, json_recon = [], [], [], []
    ...
    for epoch in range(init_epoch, args.epochs):
        ...
        if epoch + 1 in milestones_epochs or epoch == (args.epochs - 1):
            ...
            mae_train, ssim_train, psnr_train = vae_recon_test(vae, train_queue, args)
            mae_test, ssim_test, psnr_test = vae_recon_test(vae, valid_queue, args)
            json_recon.append({
                "epoch": epoch + 1,
                "mae_train": float(np.mean(mae_train)),
                "ssim_train": float(np.mean(ssim_train)),
                "psnr_train": float(np.mean(psnr_train)),
                "mae_test": float(np.mean(mae_test)),
                "ssim_test": float(np.mean(ssim_test)),
                "psnr_test": float(np.mean(psnr_test)),
            })

            # test model for interpolation
            for interp_type in ["linear", "spherical"]:
    ...
    f = open(args.save + "/acc_linear", "w")
    json.dump(json_linear, f, indent=4)
    f.close()
    f = open(args.save + "/acc_spherical", "w")
    json.dump(json_spherical, f, indent=4)
    f.close()
    f = open(args.save + "/acc_common", "w")
    json.dump(json_common, f, indent=4)
    f.close()
    f = open(args.save + "/acc_recon", "w")
    json.dump(json_recon, f, indent=4)
    f.close()
