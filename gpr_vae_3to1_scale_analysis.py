...
"""
Reconstruct images and freeze different number of top scales.
"""
...
def main(eval_args):
    ...
    dst_folder += eval_args.checkpoint.split("./checkpoints/")[-1].split("/vae")[0] + "/scale_analysis/"
    ...
    # input_paths = ["/30/11.jpg", "/27/10.jpg", "/27/0.jpg", "/30/7.jpg"]
    # Same images but loaded differently
    inputs = [ds.normal_interp_part[63], ds.normal_interp_part[19], ds.extrapolation_part[0], ds.normal_interp_part[50]]
    n_samples = len(inputs)
    trans = transforms.Compose([transforms.ToTensor()])

    # Calculate all latents
    eps_lists = []
    for row in inputs:
        ...
        eps_lists.append(all_eps)

    n_scales = args.num_latent_scales
    groups_per_scale = args.num_groups_per_scale

    # Generate initial images
    imgs = []
    for i in range(n_samples):
        img = vae.sample(1, 1.0, eps_z=eps_lists[i])
        imgs.append(img)

    perm_dims = (0, 2, 3, 1)
    fig, axs = plt.subplots(n_scales, n_samples, figsize=(20, 20))
    im_ind = 0
    for i in range(n_scales):
        if i > 0:
            # Set scales to same as first image's scale starting from the top scale
            for eps in eps_lists[1:]:
                for j in range(groups_per_scale):
                    ind = (i - 1) * groups_per_scale + j
                    eps[ind] = eps_lists[0][ind]

            imgs = [imgs[0]]
            for k in range(1, n_samples):
                img = vae.sample(1, 1.0, eps_z=eps_lists[k])
                imgs.append(img)

        for l in range(n_samples):
            img = torch.permute(imgs[l], perm_dims).cpu().numpy() * 255
            img = np.round(img[0], 0).astype("uint8")
            img = Image.fromarray(img)
            im_path = dst_folder + str(im_ind) + ".png"
            img.save(im_path)
            im_ind += 1
            axs[i, l].imshow(img)
            axs[i, l].axis("off")
