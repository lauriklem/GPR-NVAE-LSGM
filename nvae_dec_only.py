...
from layers.neural_operations import EncCombinerCell, DecCombinerCell, Conv2D, InputSkipCombinerCell, BNSwishConv, \
    UpSample
...
class NVAE_dec(nn.Module):
    def __init__(self, args, arch_instance):
        super(NVAE_dec, self).__init__()
        ...
        _, scale_ind = self.init_pre_process(scale_ind=0)

        _, _, scale_ind = self.init_encoder_tower(scale_ind)

        self.with_nf = args.num_nf > 0
        self.num_flows = args.num_nf

        # _, _, _, self.eps_conv = self.init_normal_sampler(scale_ind)

        self.dec_secondary = self.init_secondary_decoder_tower(scale_ind)
        self.prior_ftr_secondary = nn.Parameter(torch.rand(size=prior_ftr0_size), requires_grad=True)

        _, scale_ind = self.init_decoder_tower(scale_ind)
        ...
        for s in range(self.num_latent_scales):
            ...
            if s < self.num_latent_scales - 1:
                ...
                if self.progressive_input == 'input_skip':
                    progressive_input_cells.append(
                        InputSkipCombinerCell(num_co, num_ci_scale, num_co, cell_type='combiner_enc'))
    ...
    def init_secondary_decoder_tower(self, scale_ind):
        # create decoder tower
        dec_tower = nn.ModuleList()
        for s in range(self.num_latent_scales):
            for g in range(self.groups_per_scale[self.num_latent_scales - s - 1]):
                # add decoder combiner (+)
                num_c = int(self.num_channels_dec * self.channels_mult[scale_ind])
                cell = DecCombinerCell(num_c, 2 * self.num_latent_per_group + 1, num_c, cell_type='combiner_dec')
                dec_tower.append(cell)
    ...
    def calculate_eps_and_z(self, x):
        s = self.stem(x)
        # print("#####")
        # perform pre-processing
        for cell in self.pre_process:
            s = cell(s)
        # print("After preprocessing {}".format(s.size()))
        if self.progressive_input == 'input_skip':
            input_pyramid = s
            progressive_input_index = 0
        ...
        for cell in self.enc_tower:
            if cell.cell_type == 'combiner_enc':
                ...
                if cell.cell_type == 'down_enc' and self.progressive_input == 'input_skip':
                    ...
                    s, input_pyramid = self.progressive_input_cells[progressive_input_index](s, input_pyramid)
                    progressive_input_index += 1
            # print("Encoder tower {}".format(s.size()))
        # reverse combiner cells and their input for decoder
        combiner_cells_enc.reverse()
        combiner_cells_s.reverse()

        idx_dec, nf_offset = 0, 0
        all_q, all_log_q, all_eps, all_z = [], [], [], []
        ftr_enc = s
        batch_size = s.shape[0]
        s = self.prior_ftr0.unsqueeze(0)
        s = s.expand(batch_size, -1, -1, -1)
        # print("Decoder initial {}".format(s.size()))
        for cell in self.dec_tower:
            if cell.cell_type == 'combiner_dec':
                # form the encoder
                if idx_dec > 0:
                ...
                # main decoder tower
                s = cell(s)
            # print("Decoder tower {}".format(s.size()))
        return all_eps, all_z

    def sample_with_z(self, all_z=None, enable_autocast=False):
        with torch.no_grad():
            with autocast(enable_autocast):
                num_z_given = len(all_z) if all_z is not None else 0
                s = self.prior_ftr0.unsqueeze(0)
                s = s.expand(1, -1, -1, -1)
                idx_dec = 0
                for cell in self.dec_tower:
                    if cell.cell_type == 'combiner_dec':
                        if idx_dec < num_z_given:
                            z = all_z[idx_dec]
                        else:
                            b, _, h, w = s.shape
                            size = [b, self.num_latent_per_group, h, w]
    ...
    def forward_secondary_dec(self, all_z):
        s = self.prior_ftr_secondary.unsqueeze(0)
        s = s.expand(1, -1, -1, -1)
        idx_dec = 0
        for cell in self.dec_secondary:
            if cell.cell_type == 'combiner_dec':
                z = all_z[idx_dec]
                s = cell(s, z)
                idx_dec += 1
    ...
    def sample_secondary_decoder(self, all_z=None, enable_autocast=False):
        with torch.no_grad():
            with autocast(enable_autocast):
                num_z_given = len(all_z) if all_z is not None else 0
                s = self.prior_ftr_secondary.unsqueeze(0)
